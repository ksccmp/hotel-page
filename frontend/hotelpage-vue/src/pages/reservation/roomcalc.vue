  ...
  <div>
    <v-row justify="center">
      <v-col cols="6">
        ...
        <v-text-field label="예약자 이름" name="name" v-model="reservation.name" :rules="rules"></v-text-field>
        <v-text-field label="예약자 번호" name="phone" v-model="reservation.phone" :rules="rules"></v-text-field>
        <v-text-field label="추가인원" name="addperson" v-model="addperson" :rules="rules" max="3"></v-text-field>
        <v-row>
          <v-col col="6">
            <v-btn
              class="walk"
  ...
  mounted() {
    axios
      ...
      .then(res => {
        ...
        this.addprice =
          this.form == "대실" ? this.room.rentaddprice : this.room.fulladdprice;
      })
      .catch(e => {
        console.log(e);
      });
