<template>
    <v-dialog v-model="dialog" persistent width="30em">
        <template v-slot:activator="{ on }">
            <v-title class="rightnavtitle" v-on="on">회원가입</v-title>
        </template>
        <v-card class="logincard">
            <v-title>회원가입</v-title>
            <v-text-field label="아이디" name="userid" v-model="user.userid" :rules="rules"></v-text-field>
            <v-text-field label="비밀번호" name="password" v-model="user.password" type="password" :rules="rules"></v-text-field>
            ...
            <v-card-actions class="cardactions" justify-content="center">
                <v-btn color="blue darken-2" text @click="clickSignup"><b>회원가입</b></v-btn>
