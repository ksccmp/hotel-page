<template>
    <v-dialog v-model="dialog" persistent width="30em">
        <template v-slot:activator="{ on }">
            <v-title class="rightnavtitle" v-on="on">로그인</v-title>
        </template>
        <v-card class="logincard">
            <v-title>로그인</v-title>
            <v-text-field label="아이디" name="userid" v-model="user.userid" :rules="rules"></v-text-field>
            <v-text-field type="password" label="비밀번호" name="password" v-model="user.password" :rules="rules"></v-text-field>
            <v-card-actions class="cardactions" justify-content="center">
                <v-btn color="blue darken-2" text @click="login"><b>로그인</b></v-btn>
