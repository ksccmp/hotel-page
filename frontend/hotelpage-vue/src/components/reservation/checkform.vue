...
export default {
  ...
  mounted() {
    this.reservation.startdate = moment(this.reservation.startdate).format(
      "YYYY-MM-DD(dd)"
    );
    this.reservation.enddate = moment(this.reservation.enddate).format(
      "YYYY-MM-DD(dd)"
    );

    this.overdates =
      new Date(this.reservation.startdate + "21:00").getTime() >
      new Date().getTime()
        ? false
        : true;
  },
  ...
  methods: {
    reservationDelete() {
      ...
      axios
        .delete(
          "http://localhost:8080/reservation/deleteById/" + this.reservation.id,
          {
            headers: {
